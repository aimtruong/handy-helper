...
router.get('/', async (req, res) => {
    try { 
        const dbHandymanData = await Handyman.findAll({
            include: [
                {
                    model: NewListing,
                    attributes: ['title', 'post_content', 'price', 'handyman_id' ]
                }
            ]
        })
        const listings = dbHandymanData.map(listings => 
            listings.get({ plain: true }))
